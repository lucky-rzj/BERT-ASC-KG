...
# 加载数据集自适应阈值（实际使用时可通过验证集调参得到）
DATASET_THRESHOLDS = {
    'semeval': {'base': 0.3, 'step': 0.05, 'best': 0.3},  # 示例值，需通过验证集确定
    'sentihood': {'base': 0.4, 'step': 0.05, 'best': 0.4}  # 示例值，需通过验证集确定
}
...
# 新增FAISS索引构建函数（处理OOV词）
def build_faiss_index(model):
    """构建FAISS索引用于OOV词的近义种子词匹配"""
    words = list(model.wv.index_to_key)
    vectors = np.array([model.wv[word] for word in words], dtype=np.float32)
    vectors = normalize(vectors)  # 归一化向量
    index = faiss.IndexFlatIP(vectors.shape[1])  # 内积索引（等价于余弦相似度，因为已归一化）
    index.add(vectors)
    return index, words

# 新增OOV词处理函数
def find_similar_seeds(oov_word, index, words, model, top_k=5):
    """为OOV词寻找相似的种子词"""
    # 简单处理：如果是复合词，拆分后取向量平均（可根据实际情况优化）
    if ' ' in oov_word:
        tokens = oov_word.split()
    else:
        tokens = [oov_word]
    ...
    # 计算平均向量
    avg_vector = np.mean(valid_vectors, axis=0, keepdims=True).astype(np.float32)
    avg_vector = normalize(avg_vector)
    
    # FAISS搜索
    distances, indices = index.search(avg_vector, top_k)
    similar_words = [words[i] for i in indices[0]]
    return similar_words

def semeval(threshold=None):
    # 加载模型并构建FAISS索引
    model = gensim.models.Word2Vec.load(r'embeddings/restaurant.bin')
    faiss_index, faiss_words = build_faiss_index(model)
    
    # 使用数据集自适应阈值
    if threshold is None:
        threshold = DATASET_THRESHOLDS['semeval']['best']
            ...
            for a in aspect_category:
                ...
                for w in text_w_stops:
                    ...
                    if w not in model.wv:
                        # 寻找相似种子词
                        similar_seeds = find_similar_seeds(w, faiss_index, faiss_words, model)
                        # 过滤不属于当前类别的种子词
                        similar_seeds = [s for s in similar_seeds if s in category_seed_words.get(a, [])]
                        if similar_seeds:  # 如果找到匹配的种子词，直接加入代表词
...
def sentihood(threshold=None):
    ...
    # 使用数据集自适应阈值
    if threshold is None:
        threshold = DATASET_THRESHOLDS['sentihood']['best']
...
def main():
    ...
    parser.add_argument('--threshold', type=float, help='Override the default threshold')  # 新增阈值参数
    opt = parser.parse_args()
    
    if opt.dataset =='semeval':
        semeval(threshold=opt.threshold)
    else:
        sentihood(threshold=opt.threshold)
