...
from data_utils import(
    ...
    ABSADataset_absa_bert_semeval_json,    # SemEval数据集加载器
    ABSADataset_absa_bert_sentihood_json   # Sentihood数据集加载器
)  
from evaluation import *      #评估函数
from MyModel_adaptive import BERT_ASC_adaptive   #核心模型
...
class Instructor:
    ...
    def __init__(self, opt):
        ...
        #self.model = BERT_ASC_adaptive.from_pretrained(opt.pt_model, num_labels=opt.label_dim, task_mode=opt.task_mode)
        self.model = BERT_ASC_adaptive.from_pretrained(opt.pretrained_bert_name, num_labels=opt.label_dim, task_mode=opt.task_mode)
        ...
        if self.opt.dataset=='semeval':
            ...
            if opt.valset_ratio > 0:
                ...
                logger.info(f"[SemEval] Using {opt.valset_ratio*100:.0f}% train as valset")
            else:
                valset_len = int(0.1 * len(self.trainset))
                self.trainset, self.valset = random_split(self.trainset, (len(self.trainset)-valset_len, valset_len))
        else:
            ...
            logger.info("[Sentihood] Loaded train/valset/test")
    ...
    def _train(self, optimizer, train_data_loader, val_data_loader, t_total):
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(tqdm(train_data_loader)):
                ...
                #解包批次数据: input_ids：token的索引序列、token_type_ids：句子分段ID、attention_mask：注意力掩码、 labels：情感标签
            ...
            if self.opt.dataset  == 'semeval':
                ...
                if self.opt.task_mode == "implicit_aspect":
                    max_per = aspect_F                  # 隐式方面 → 用 F1 
                else:
                    max_per = sentiment_Acc_4_classes   # 情感极性 → 用 4类准确率
            else:
                ...
                # ----------- ⭐ 根据任务模式选择最佳指标 -----------
                if self.opt.task_mode == "implicit_aspect":
                    max_per = aspect_strict_Acc      # 隐式方面 → Strict Acc
                else:
                    max_per = sentiment_Acc          # 情感极性 → Sentiment Acc

            # ------------------ ✅ PRINT VALIDATION METRICS BY TASK ------------------
            if self.opt.dataset == 'semeval':
                if self.opt.task_mode == 'implicit_aspect':    # Aspect Detection        
                    logger.info("******************** SemEval Validation — Aspect Detection *******************")
                    logger.info('>> P: {:.4f} , R: {:.4f} , F: {:.4f}'.format(aspect_P, aspect_R, aspect_F))
                    logger.info("********************************************************************************")
                else:    # Sentiment Classification
                    logger.info("******************** SemEval Validation — Sentiment Classification ************")
                ...
                if self.opt.task_mode == 'implicit_aspect':     
                    logger.info("**************** Sentihood Validation — Aspect Detection **********************")
                    ...
                    logger.info("**************** Sentihood Validation — Sentiment Classification **************")
            ...
            if max_per > max_val_f1:   #当前验证集性能 > 历史最佳
                ...
                if not os.path.exists('state_dict'):
                    os.mkdir('state_dict')
                best_state = copy.deepcopy(self.model.state_dict())   #深拷贝模型当前参数
                
    
            self.model.train() 
        logger.info(f"🔥 Training Finished. Best Epoch = {best_epoch}")
        return best_state, best_epoch    #返回最佳模型的参数字典、最优轮次
    ...
    def run(self):
        ...
        # 2. 将训练集转换为TensorDataset格式，包含4个张量：token索引、句子分段ID、注意力掩码、标签张量
        ...
        best_state, best_epoch = self._train(optimizer, train_data_loader, val_data_loader, t_total)
        ...
        if self.opt.save_model:
            #构建模型保存路径-----按照任务分类保存
            save_path = f"/hy-tmp/BERT-ASC-main/code/BERT_ASC_adaptive/state_dict/{self.opt.dataset}/{self.opt.task_mode}/seed{self.opt.seed}.bm"
