...
class AdaptiveLayerFusion(nn.Module):
    ...
    def __init__(
        ...
        # 任务向量：隐式方面 / 极性判断
        self.e_implicit = nn.Parameter(torch.randn(self.hidden_size))
        self.e_sentiment = nn.Parameter(torch.randn(self.hidden_size))
    ...
    def forward(self, layers, attention_mask, labels=None):
        ...
        if self.task_mode.lower() == "implicit_aspect":
            e_task = self.e_implicit  # [H]
            lambda_sem = 0.8     #语义偏置更强
        else:
            # 默认 sentiment_polarity
...
class BERT_ASC_adaptive(BertPreTrainedModel):
    ...
    """
    def __init__(self, config, num_labels=None, task_mode: str = "polarity"):
